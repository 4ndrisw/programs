...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        elseif ($aColumns[$i] == '1') {
            $_data = '<a class="btn btn-success" title = "'._l('propose_this_item').'" href="#" onclick="programs_add_program_item(' . $clientid . ','. $inspector_id . ','. $inspector_staff_id  . ','. $surveyor_id . ','. $program_id . ',' . $aRow['id'] . ',' . '); return false;">+</a>';
        }
        $row[] = $_data;
    }
