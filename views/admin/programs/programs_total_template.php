...
<div class="row">
<?php if(count($programs_years) > 1 || isset($currencies)){ ?>
  <div class="col-md-12 simple-bootstrap-select mbot5">
   <?php if(isset($currencies)){ ?>
    <select class="selectpicker" data-width="auto" name="total_currency" onchange="init_program_total();">
    ...
    <?php } ?>
      <?php
      if(count($programs_years) > 1){ ?>
      <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker" multiple name="programs_total_years" onchange="init_program_total();">
  ...
  foreach($totals as $key => $data){
    $class = program_state_color_class($data['state']);
    $name = program_state_by_id($data['state']);
    ?>
