...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-'.$program->id; ?>" id="program_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<?php echo form_open('admin/programs/send_to_email/'.$program->id); ?>
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close close-send-template-modal"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel">
					<span class="edit-title"><?php echo _l('program_send_to_client_modal_heading'); ?></span>
				</h4>
			</div>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="form-group">
							...
							$selected = array();
							$contacts = $this->clients_model->get_contacts($program->clientid,array('active'=>1,'program_emails'=>1));
							foreach($contacts as $contact){
								array_push($selected,$contact['id']);
							}
							 if(count($selected) == 0){
                                    echo '<p class="text-danger">' . _l('sending_email_contact_permissions_warning',_l('customer_permission_program')) . '</p><hr />';
                                }
							echo render_select('sent_to[]',$contacts,array('id','email','firstname,lastname'),'invoice_program_sent_to_email',$selected,array('multiple'=>true),array(),'','',false);
							?>
						</div>
						...
						<div class="checkbox checkbox-primary">
							...
							<label for="attach_pdf"><?php echo _l('program_send_to_client_attach_pdf'); ?></label>
						</div>
						<h5 class="bold"><?php echo _l('program_send_to_client_preview_template'); ?></h5>
						<hr />
						<?php echo render_textarea('email_template_custom','',$template->message,array(),array(),'','tinymce-'.$program->id); ?>
