...
class Programs extends AdminController
{
    ...
    public function get_program_data_ajax($id, $to_return = false)
    {
        ...
        if ($program->inspectionid !== null) {
            $this->load->model('invoices_model');
            $program->invoice = $this->invoices_model->get($program->inspectionid);
        }
    ...
    public function convert_to_inspection($id)
    {
        ...
        $inspectionid = $this->programs_model->convert_to_inspection($id, false, $draft_inspection);
        if ($inspectionid) {
            set_alert('success', _l('program_convert_to_inspection_successfully'));
            redirect(admin_url('inspections/list_inspections/' . $inspectionid));
        } else {
